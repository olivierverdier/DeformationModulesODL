...
class UnconstrainedAffine(DeformationModule):
    def __init__(self,DomainField, NAffine, Kernel):
        """Initialize a new instance.
        DomainField : space on wich vector fields will be defined
        NAffine : number of affine deformations
        ...
        if(self.dim==2):
            ...
            print('UnconstrainedAffine only for dimension 2 or 3')

        self.Directions=Directions.copy()

        # Each affine deformation is defined by a centre (a point, GD) and
        # dim+1 vectors (the controls)
        GDspace=odl.ProductSpace(odl.space.rn(self.dim),self.NAffine)
        Contspace=odl.ProductSpace(odl.ProductSpace(odl.space.rn(self.dim),self.dim+1),self.NAffine)
    ...
    def ComputeToolPoints(self,o):
         """ The affine deformations are built as sum of local translations
         centred around the GDs. Here we compute these points"""

         TP=self.Contspace.element()
         for i in range(self.NAffine):
             TP[i]=(o[i]+self.Directions).copy()
    ...
    def ComputeField(self, o,h):
        ...
        for i in range(self.NAffine):
            for u in range(self.dim +1):
                ...
                vector_field += self.DomainField.tangent_bundle.element([kern * hu for hu in h[i][u]])
    ...
    def ComputeFieldEvaluate(self):
        ...
        class Eval(Operator):
            ...
            def _call(self,x):
                ...
                for i in range(ope.NAffine):
                    for u in range(ope.NAffine):
                        a=ope.Kernel(self.TP[i][u]-x)
                        speed+=a*self.Cont[i][u]
    ...
    def Cost(self,GD,Cont):
        ...
        for i in range(self.NAffine):
            for j in range(self.NAffine):
                for u in range(self.dim+1):
                    for v in range(self.dim+1):
                        fac=self.Kernel(TP[i][u]-TP[j][v])
                        prod=[hi*hj for hi, hj in zip(Cont[i][u],Cont[j][v])]
                        energy+=fac*sum(prod)
        return energy
