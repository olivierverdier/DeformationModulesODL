...
class LocalScaling(DeformationModule):
    def __init__(self,DomainField, NScaling, Kernel):
        """Initialize a new instance.
        DomainField : space on wich vector fields will be defined
        NScaling : number of NScalings
        Kernel : kernel, class that has at least methods Eval and derivative
        """

        self.NScaling=NScaling
        ...
        if(self.dim==2):
            ...
            print('LocalScaling only for dimension 2 or 3')
        ...
        GDspace=odl.ProductSpace(odl.space.rn(self.dim),self.NScaling)
        Contspace=odl.space.rn(self.NScaling)

        basis=[]
        for i in range(self.NScaling):
    ...
    def ComputeToolPoints(self,o):
         """ The Scaling deformations are built as sum of local translations
         centred around the GDs. Here we compute these points"""

         TP=odl.ProductSpace(odl.ProductSpace(odl.space.rn(self.dim),self.dim+1),self.NScaling).element()
         for i in range(self.NScaling):
    ...
    def ComputeFieldEvaluate(self):
        ...
        class Eval(Operator):
            ...
            def _call(self,x):
                ...
                for i in range(ope.NScaling):
                    for u in range(ope.NScaling):
